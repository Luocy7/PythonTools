...
from Mooc.Icourse163.Icourse163Base import *
...
class Icourse163Mooc(Icourse163Base):
    ...
    def _download(self):  # 根据课程视频链接来下载高清MP4慕课视频, 成功下载完毕返回 True
        ...
        Icourse163Base.potplayer.init(rootDir)
        Icourse163Base.potplayer.enable()
        for i, chapter in enumerate(self.infos, 1):  # 去除 win 文价夹中的非法字符
            ...
            for j, lesson in enumerate(self.infos[chapter], 1):
                ...
                for k, pdf_source in enumerate(sources['pdfs'], 1):
                    ...
                    if pdf_url:
                        self.download_pdf(pdf_url, pdf_name, lesson_dir)
                ...
                for k, video_source in enumerate(sources['videos'], 1):
                    ...
                    if video_url:
                        self.download_video(video_url=video_url, video_name=video_name, video_dir=lesson_dir)
                    if sub_url:
                        self.download_sub(sub_url, sub_name, lesson_dir)
