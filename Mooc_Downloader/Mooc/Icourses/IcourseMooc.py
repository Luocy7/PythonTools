...
class IcourseMooc(IcourseBase):
    ...
    def _get_title(self):
        ...
        match_school = re.search(
            r'<span +class="pull-left">学校:</span>\s*<p +class="course-information-hour pull-left">(.*?)</p>', text)
    ...
    def _get_infos(self):
        ...
        unit_list = re.findall(
            r'<a +class="chapter-body-content-text section-event-t no-load"[\s\S]*?data-secId="(\d+)"[\s\S]*?<span +class="chapter-s">(\d+)</span><span>.</span>\s*?<span +class="chapter-t">(\d+)</span>(.*?)</a>',
            text1)
    ...
    def _download(self):
        ...
        if (self.mode & IS_MP4) or (self.mode & IS_PDF):
            ...
            for cnt1, info in enumerate(self.infos, 1):
                ...
                print(chapter)
                chapter_dir = os.path.join(course_dir, chapter)
                if not os.path.exists(chapter_dir):
                    os.mkdir(chapter_dir)
                mp4_list, pdf_list = self._get_course_links(info['id'])
                pdf_list += [(pdf['url'], pdf['name']) for pdf in info['pdfs']]
                if self.mode & IS_PDF:
                    self.download_pdf_list(chapter_dir, pdf_list, '{}.'.format(cnt1))
                if self.mode & IS_MP4:
                    self.download_video_list(chapter_dir, mp4_list, '{}.'.format(cnt1))
                for cnt2, unit in enumerate(info['units'], 1):
                    lesson = '{' + str(cnt2) + '}--' + unit['name']
                    print("  " + lesson)
                    lesson_dir = os.path.join(chapter_dir, lesson)
                    if not os.path.exists(lesson_dir):
                        os.mkdir(lesson_dir)
                    mp4_list, pdf_list = self._get_course_links(unit['id'])
                    pdf_list += [(pdf['url'], pdf['name']) for pdf in unit['pdfs']]
                    if self.mode & IS_PDF:
                        self.download_pdf_list(lesson_dir, pdf_list, '{}.{}.'.format(cnt1, cnt2))
                    if self.mode & IS_MP4:
                        self.download_video_list(lesson_dir, mp4_list, '{}.{}.'.format(cnt1, cnt2))
        if self.mode & IS_PAPER:
            paper_dir = os.path.join(rootDir, PAPERNAME)
            if not os.path.exists(paper_dir):
                os.mkdir(paper_dir)
            print("-" * LEN_ + "下载试卷" + "-" * LEN_)
            paper_list = self._get_paper_links()
            self.download_pdf_list(paper_dir, paper_list)
        if self.mode & IS_SOURCE:
            source_dir = os.path.join(rootDir, SOURCENAME)
            if not os.path.exists(source_dir):
                os.mkdir(source_dir)
            print("-" * LEN_ + "下载资源" + "-" * LEN_)
            IcourseBase.potplayer.disable()
            ...
            self.download_pdf_list(source_dir, pdf_list)
            self.download_video_list(source_dir, mp4_list)
